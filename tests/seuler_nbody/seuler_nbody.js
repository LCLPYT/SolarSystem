...
// test(24 * 60 * 60, 60, false, EARTH_REAL_2020_09_02);
test(365 * 24 * 60 * 60, 120, false, EARTH_REAL_2021_09_01);
...
function test(duration, dt, print = false, real_pos) {
    ...
    for (t = 0; t < duration; t += dt) {
        PLANETS.list.forEach(body1 => {
            body1.accel = new Vector(0, 0, 0);
            PLANETS.list.forEach(body2 => {
                if(body1 !== body2)
                  body1.accel = body1.accel.add(accel(body2, body1));
            });
            body1.vel = body1.vel.add(body1.accel.multScalar(dt));
            body1.pos = body1.pos.add(body1.vel.multScalar(dt));
            if(print) body1.draw(ctx, canvas);
        });
    }
    console.log('Done.');
...
function accel(body1, body2) {
    let dir = body2.pos.sub(body1.pos);
    let scalar = -grav * body1.mass / dir.lengthSquared();
    return dir.normalize().multScalar(scalar);
  }
